...
protected:
    nokia::unique_ptr<double> testUniquePtr_;
};
...
TEST(UniquePtrTestArrowOperator, ArrowOperatorShouldGuaranteeAcessToStructuresAndUnions) {
    nokia::unique_ptr<UniquePtrArrowOperatorTest> testUniquePtr_(new UniquePtrArrowOperatorTest{});
...
TEST_F(UniquePtrTest, UniquePtrShouldMovingOwnershipToAnotherUnqiuePtr) {
    nokia::unique_ptr<double> testNewPtr = std::move(testUniquePtr_);
